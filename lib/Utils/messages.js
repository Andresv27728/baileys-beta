...
export const prepareWAMessageMedia = async (message, options) => {
  ...
  if (isNewsletter) {
    ...
    if (message.isLottie) {
      obj = { lottieStickerMessage: { message: obj } };
    }
    if (cacheableKey) {
...
export const generateWAMessageContent = async (message, options) => {
  ...
  if (hasOptionalProperty(message, "interactiveMessage") && message.interactiveMessage) {
    m.interactiveMessage = {
      body: { text: message.interactiveMessage.body?.text || message.interactiveMessage.body || '' },
      footer: { text: message.interactiveMessage.footer?.text || message.interactiveMessage.footer || '' },
      header: {
        title: message.interactiveMessage.header?.title || '',
        subtitle: message.interactiveMessage.header?.subtitle || '',
        hasMediaAttachment: false,
        ...(message.interactiveMessage.header?.imageMessage ? { imageMessage: message.interactiveMessage.header.imageMessage } : {}),
        ...(message.interactiveMessage.header?.videoMessage ? { videoMessage: message.interactiveMessage.header.videoMessage } : {}),
        ...(message.interactiveMessage.header?.documentMessage ? { documentMessage: message.interactiveMessage.header.documentMessage } : {}),
      },
      nativeFlowMessage: {
        buttons: (message.interactiveMessage.nativeFlowMessage?.buttons || []).map(btn => ({
          name: btn.name,
          buttonParamsJson: typeof btn.buttonParamsJson === 'string' ? btn.buttonParamsJson : JSON.stringify(btn.buttonParamsJson || {}),
        })),
        messageParamsJson: message.interactiveMessage.nativeFlowMessage?.messageParamsJson || '{}',
      },
    };
  }
  if (hasOptionalProperty(message, "nativeFlow") && message.nativeFlow) {
    const buttons = (Array.isArray(message.nativeFlow) ? message.nativeFlow : message.nativeFlow.buttons || []).map(btn => {
      if (btn.id) {
        return { name: 'quick_reply', buttonParamsJson: JSON.stringify({ display_text: btn.text || btn.display_text || '', id: btn.id }) };
      }
      if (btn.url) {
        return { name: 'cta_url', buttonParamsJson: JSON.stringify({ display_text: btn.text || btn.display_text || '', url: btn.url, webview_interaction: false }) };
      }
      if (btn.copy) {
        return { name: 'cta_copy', buttonParamsJson: JSON.stringify({ display_text: btn.text || btn.display_text || '', copy_code: btn.copy }) };
      }
      if (btn.call) {
        return { name: 'cta_call', buttonParamsJson: JSON.stringify({ display_text: btn.text || btn.display_text || '', phone_number: btn.call }) };
      }
      if (btn.sections) {
        return { name: 'single_select', buttonParamsJson: JSON.stringify({ title: btn.title || '', sections: btn.sections }) };
      }
      return { name: btn.name || 'quick_reply', buttonParamsJson: typeof btn.buttonParamsJson === 'string' ? btn.buttonParamsJson : JSON.stringify(btn.buttonParamsJson || {}) };
    });
    m.interactiveMessage = {
      body: { text: message.text || '' },
      footer: { text: message.footer || '' },
      header: { title: message.title || '', subtitle: message.subtitle || '', hasMediaAttachment: false },
  ...
  if (hasOptionalProperty(message, "interactiveAsTemplate") && message.interactiveAsTemplate) {
    const templateId = message.templateId || `template-${Date.now()}`;
    const interactiveContent = { ...m };
    m = {
      templateMessage: {
        interactiveMessageTemplate: interactiveContent,
...
export const extractMessageContent = (content) => {
  ...
  if (content?.templateMessage?.interactiveMessageTemplate) {
    return content.templateMessage.interactiveMessageTemplate;
  }
  return content;
};
/**
